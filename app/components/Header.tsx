...
function Header() {
  ...
  return (
    <header className='flex px-6 xl:px-0 justify-between items-center pt-2 md:pt-12 container mx-auto z-50'>
      <Link href={'/'}>
        <Image
          src="/logo.png"
      ...
      <div className=" gap-8 text-2xl hidden md:flex text-white font-harmonia">
        {nav_items.map((item) => 
          item.id === 'how-it-works' ? (
            <Dialog key={item.id} open={isDialogOpen} onOpenChange={setIsDialogOpen}>
              ...
              <DialogContent className='p-0 border-none w-[90%] xl:w-full'>
                <DialogHeader className='p-0 rounded-xl'>
                  ...
                  <DialogDescription className="sr-only">Watch our video to learn how Santa Phone Calls work</DialogDescription>
      ...
      <div className='block md:hidden'>
        <DropdownMenu>
          ...
          <DropdownMenuContent className='bg-gradient-to-r from-[#D7C798] via-[#EDE4CC] to-[#D7C798] relative right-[20%] z-[100]'>
            {nav_items.map((item) => (
              <DropdownMenuItem key={item.name} className='font-bold border-b-2 border-[#D7C798]/30 px-8 text-xl py-4 focus:outline-none font-seasons group'>
                {item.id === 'how-it-works' ? (
                  <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                    <DialogTrigger asChild>
                      <button onClick={() => setIsDialogOpen(true)} className="w-full text-left transition-all duration-300 group-hover:translate-x-2 group-hover:text-[#8B7355]">
                        {item.name}
                      </button>
                    </DialogTrigger>
                    <DialogContent className='p-0 border-none w-[90%] xl:w-full'>
                  ...
                  <Link href={item.href} className={`${path === item.href ? 'font-bold' : ''} transition-all duration-300 group-hover:translate-x-2 group-hover:text-[#8B7355] w-full block`}>
                    {item.name}
                  </Link>
                )}
              </DropdownMenuItem>
